...
A network flow-based integer programming model was formulated to estimate the minimum fleet size required to operate the scheduled Southwest Airlines flights on January 25, 2026. Each flight is represented as a node in a directed network, and feasible aircraft connections between flights are represented as arcs. A connection is feasible when the destination airport of one flight matches the origin airport of another flight and the available ground time satisfies the minimum turnaround requirement.  
...
Improving fleet utilization allows an airline to operate a fixed flight schedule with fewer aircraft, which can reduce the amount of capital tied up in expensive physical assets. Even when total passenger flight fuel consumption remains largely unchanged, using aircraft more efficiently can lower ownership or leasing costs, parking and storage costs, insurance costs, and some maintenance overhead. Higher utilization also helps airlines better understand whether their existing fleet is being used effectively or whether some aircraft are underused. In this project, minimizing the number of aircraft provides a theoretical lower bound on fleet requirements and highlights how routing decisions can improve aircraft productivity under fixed schedule and turnaround-time assumptions. It is not a direct replacement for Southwest's actual aircraft assignment, since the model does not include crew scheduling, maintenance, aircraft capacity, gate availability, or multi-day positioning constraints.
